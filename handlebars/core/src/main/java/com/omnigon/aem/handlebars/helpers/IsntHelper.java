 ...
 *
 * {{#isnt foo 'bar'}}
 * This content will be rendered if property foo<string> does NOT equals to string 'bar'
 * {{/is}}
...
@Service
@Component
public class IsntHelper implements HandlebarsHelper {

    private static final String NAME = "isnt";
    ...
    public CharSequence apply(final Object target, final Options options) throws IOException {
        ...
        if(!target.equals(options.param(0))) {
            content = options.fn();
            //handle case of no inner content.
